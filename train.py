    ...
    while True:
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0:
            ...
            with open(f"./saved_models/{opt.exp_name}/log_train.txt", "a") as log:
                ...
                best_model_log = (
                    f'{"Best_accuracy":17s}: {best_accuracy:0.3f}, {"Best_norm_ED":17s}: {best_norm_ED:0.2f}'
                )
...
if __name__ == "__main__":
    ...
    if not opt.exp_name:
        ...
        current_time = f"{current_time.tm_year}.{current_time.tm_mon}.{current_time.tm_mday} {current_time.tm_hour}{current_time.tm_min}"
        opt.exp_name = (
            f"{current_time} {opt.Transformation}-{opt.FeatureExtraction}-{opt.SequenceModeling}-{opt.Prediction}"
        )
